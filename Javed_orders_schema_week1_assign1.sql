...
# 1.	Write a query to Display the PRODUCT_ID ,PRODUCT_DESC, PRODUCT_CLASS_CODE,PRODUCT_PRICE 
...
# 2.	Write a query to display the Customer_id,order id, order_date and 
#orders shipment date in an ascending order of customer ids and 
# descending order of order dates.(70 rows) 

select customer_id, order_id, order_date, ORDER_SHIPMENT_DATE from order_header 
order by ORDER_SHIPMENT_DATE,CUSTOMER_ID,ORDER_DATE desc;

# 3. Write a query to List the details of products with class code 2050 
#where price is in the range of 10000 and 30000 and available quantity is equal 
#to or more than 15

select PRODUCT_CLASS_CODE,PRODUCT_PRICE,PRODUCT_QUANTITY_AVAIL from product
where PRODUCT_CLASS_CODE= 2050 and PRODUCT_PRICE between 10000 and 30000 and PRODUCT_QUANTITY_AVAIL>= 15;

# 4.	Write a query to display all the records of order details which are not yet to be shipped.
select * from order_header
where  ORDER_STATUS in ('In process');

# 5.	Write a query to display all the records of order details which are  shipped and payment_mode either ‘credit card’ or 
# ‘net banking’ and payment_date greater than ‘2013-01-01’.(10 rows) 

select * from order_header
where PAYMENT_MODE = ('Credit Card' and 'Net Banking') and PAYMENT_MODE != 'Cash'
and ORDER_STATUS= 'Shipped' and PAYMENT_DATE > '2013-01-01' ;

# 6. Write a query to display the all the records of customers whose creation date is 
# before ’12-Jan-2006’ and email address contains ‘gmail’ or ‘yahoo’ and user name starts with ‘dave’.

select customer_creation_date from online_customer; 
select * from online_customer
where (CUSTOMER_CREATION_DATE < '2006-01-12') 
and (customer_email like '%gmail%' ) or (customer_email like '%yahoo%') and (CUSTOMER_USERNAME like 'DAVE%');

# 7. Write query to display the product id,product_description 
# and total worth(product_price * product_quantity available) of each product

select product_id,PRODUCT_DESC, (product_price*PRODUCT_QUANTITY_AVAIL) as total_worth from product;

# 8. Write a query to Display details of customer who have Gmail account and phone number consist of ‘77’ as below:
# <Customer full name> (<customer user name>) created on <date>. Contact Phone: <Phone no.> E-mail: <E-mail id>.

select concat(customer_fname,'(',customer_username,')',
' created on ' ,customer_creation_date,'.Contact Phone:',customer_phone,'E-mail:',customer_email) as report 
from online_customer
where CUSTOMER_EMAIL like '%gmail%' and CUSTOMER_PHONE like '%77%';

# 9. Write a query to Display the product details (product_class_code, product_id, 	product_desc, product_price,) as per the following criteria and sort them in 	descending order of category:
# a.	If the category is 2050, increase the price by 2000
# b.	If the category is 2051, increase the price by 500
# c.	If the category is 2052, increase the price by 600.

select p.product_class_code,product_id,product_desc,PRODUCT_CLASS_DESC,
case 
	when p.product_class_code= 2050 then product_price + 2000
    when p.product_class_code= 2051 then product_price + 500
    when p.product_class_code= 2052 then product_price + 600
    else q.product_price
end as product_price
from product_class p
left join product q on p.product_class_code =q.product_class_code
order by product_class_code desc;


# 10. Write a Query to display the  the product description, product class description and product price of all products which are shipped.
select s.order_id,PRODUCT_CLASS_DESC,PRODUCT_DESC,PRODUCT_PRICE,ORDER_STATUS
from product_class p 
left join product q on p.PRODUCT_CLASS_CODE=q.PRODUCT_CLASS_CODE 
left join order_items r on q.product_id=r.product_id
left join order_header s on r.order_id = s.order_id
where order_status= 'Shipped';

# 11.	Write a query to display the  customer_id,customer name, email 
# and order details (order id, product desc,product  qty, subtotal(product_quantity * product_price)) 
# for all customers even if they have not ordered any item.(

select r.order_id,o.customer_id,CUSTOMER_FNAME,CUSTOMER_EMAIL,order_status, product_quantity,(PRODUCT_QUANTITY*product_price) as gross
from online_customer o 
left join order_header h on o.customer_id=h.customer_id
left join order_items r on h.order_id=r.order_id
left join product t on r.product_id=t.product_id ;

# 12.	Write a query to display the customer_id,customer full name ,city,pincode,and order details 
...
select r.order_id,a.customer_id,CUSTOMER_FNAME,CUSTOMER_EMAIL,
order_status, product_quantity,(PRODUCT_QUANTITY*product_price) as gross, city,PINCODE
from address o 
left join online_customer a on o.address_id=a.address_id 
left join order_header h on a.customer_id=h.customer_id 
left join order_items r on h.order_id=r.order_id 
left join product t on r.product_id=t.product_id 
left join product_class u on t.PRODUCT_CLASS_CODE= u.PRODUCT_CLASS_CODE
where not pincode like '%0%' and order_status ='Shipped'
order by CUSTOMER_FNAME, ORDER_DATE,gross;

# 13.	Write a query to display (customer id,customer fullname,city) of customers  
# from outside ‘Karnataka’ who haven’t bought any toys or books

select a.customer_id,customer_fname,city ,state , product_class_desc
from address o 
left join online_customer a on o.address_id=a.address_id
left join order_header h on a.customer_id=h.customer_id
left join order_items r on h.order_id=r.order_id
left join product t on r.product_id=t.product_id 
left join product_class u on t.PRODUCT_CLASS_CODE= u.PRODUCT_CLASS_CODE
where state != 'Karnataka' and product_class_desc != ('toys') and PRODUCT_CLASS_DESC != 'books' 
group by customer_fname;


# 14.	Write a query to display the customer full name and total order value(product_quantity*product_price) of 
# premium customers (i.e. the customers who bought items total worth > Rs. 1 lakh
select CUSTOMER_FNAME,t.product_price,PRODUCT_QUANTITY,(PRODUCT_QUANTITY*product_price) as gross
from online_customer o 
left join order_header h on o.customer_id=h.customer_id
left join order_items r on h.order_id=r.order_id
left join product t on r.product_id=t.product_id 
...
# 15.	Write a query to display the customer id and cutomer full name of customers along with 
# (product_quantity) as total quantity of products ordered for order ids > 10060
select r.order_id,CUSTOMER_FNAME,PRODUCT_QUANTITY
from online_customer o 
left join order_header h on o.customer_id=h.customer_id
left join order_items r on h.order_id=r.order_id
where r.order_id > 10060 and product_quantity>1;

# 16.	Write a query to display (product_class_desc, product_id, product_desc, product_quantity_avail ) and Show inventory status of products as below as per their available quantity:
...
case
when (u.PRODUCT_CLASS_DESC in ('Electronics','Computer'))and (product_quantity_avail <10) then 'lowstock'
when (u.PRODUCT_CLASS_DESC in ('Electronics','Computer')) and (PRODUCT_QUANTITY_AVAIL between 11 and 30)then 'INSTOCK'
WHEN (u.PRODUCT_CLASS_DESC in ('Electronics','Computer')) and (PRODUCT_QUANTITY_AVAIL >30 )then 'Enoughstock'
when (u.PRODUCT_CLASS_DESC in ('Stationery' , 'Clothes')) and (product_quantity_avail <20) then 'lowstock'
when  (u.PRODUCT_CLASS_DESC in ('Stationery' , 'Clothes'))and (PRODUCT_QUANTITY_AVAIL between 21 and 80) then 'INSTOCK'
WHEN  (u.PRODUCT_CLASS_DESC in ('Stationery' , 'Clothes')) and (PRODUCT_QUANTITY_AVAIL >81) then 'Enoughstock'
when (u.PRODUCT_CLASS_DESC not in ('Electronics','Computer','Stationery' , 'Clothes')) and (product_quantity_avail <15) then 'lowstock'
when  (u.PRODUCT_CLASS_DESC not in ('Electronics','Computer','Stationery' , 'Clothes'))and (PRODUCT_QUANTITY_AVAIL between 16 and 50) then 'INSTOCK'
WHEN  (u.PRODUCT_CLASS_DESC not in ('Electronics','Computer','Stationery' , 'Clothes')) and (PRODUCT_QUANTITY_AVAIL >51) then 'Enoughstock'
when PRODUCT_QUANTITY_AVAIL = 0 then 'Outofstock'
else 'Unknown'
end as inventorystatus  
from product t
left join product_class u on t.PRODUCT_CLASS_CODE= u.PRODUCT_CLASS_CODE;

# 17.	Write a Query to display the  product id, product description and product  price of products  
...
where product_id < 1000 and product_price not in (select  product_price from product group by product_price having count(PRODUCT_PRICE)=1)
order by product_price;

# 18. Write a query to display product class description ,total quantity(sum(product_quantity),
...
select PRODUCT_CLASS_DESC, max(product_quantity), product_price,
 (PRODUCT_QUANTITY* product_price) as total_value, country
from address o 
right join online_customer a on o.address_id=a.address_id
right join order_header h on a.customer_id=h.customer_id
right join order_items r on h.order_id=r.order_id
right join product t on r.product_id=t.product_id 
right join product_class u on t.PRODUCT_CLASS_CODE= u.PRODUCT_CLASS_CODE
where country not in ('India','USA') ;

# 19.	Write a query to display the customer id, customer first name, address line 2,city  
# total sales(sum(product quantity * product price (0 if they haven't purchased any item)) made by customers who stay in the same locality (i.e. same address_line2 & city)
...
# 20.	Write a Query to display customer id,customer firstname,lastname,order status,
#total value(sum(product quantity * product price)) and show who is the most valued customer (customer who made the highest sales)

select CUSTOMER_FNAME,CUSTOMER_LNAME,count(CUSTOMER_FNAME),order_status,
...
left join product t on r.product_id=t.product_id 
group by CUSTOMER_FNAME
order by count(CUSTOMER_FNAME) desc limit 1;

# 21.	Write a query to display product class code,product class desc,product id product description,
# product price and show the most expensive products in their respective classes.
select product_id, t.product_class_code, product_class_desc, product_desc, max(product_price) 
from product t
join product_class u on t.PRODUCT_CLASS_CODE= u.PRODUCT_CLASS_CODE
group by PRODUCT_CLASS_DESC
order by max(product_price) desc ;

# 22.	Write a query to display shipper id,shipper name , (len*width*height*product_quantity) as total volume shipped 
# and show Which shipper has shipped highest volume of items.
select o.shipper_id,SHIPPER_NAME,sum(LEN*WIDTH*HEIGHT*PRODUCT_QUANTITY) total_volume_shipped, ORDER_STATUS from shipper o 
join order_header h on o.shipper_id=h.shipper_id
join order_items r on h.order_id=r.order_id
join product t on r.product_id=t.product_id 
where ORDER_STATUS= 'Shipped'
group by shipper_name
order by total_volume_shipped desc limit 1;

# 23.	Write a query to display  carton id ,(len*width*height) as carton_vol and identify the optimum carton 
# (carton with the least volume whose volume is greater than the total volume of all items) for a 
# given order whose order id is 10006 , Assume all items of an order are packed into one single carton 

select carton_id,(len*width*height) as carton_vol from carton
order by carton_vol desc limit 1;


# 24.	Write a query to display product id,product description,total_quantity (sum(order_quantity) ,   
 # Provided show the most and least sold products quantity-wise

select product_id, t.product_class_code, product_class_desc, product_desc, sum(order_quantity) 
from product t
group by PRODUCT_CLASS_DESC
order by max(product_price) desc ;
